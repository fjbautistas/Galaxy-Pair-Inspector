...
--     rp ∈ [20, 50). Solo se rellenan después de que el frontend invoque
--     `claim_v2_slice` (al terminar el suplementario #150).
--   • `assign_partition` mantiene su contrato legacy + devuelve los 3 campos
--     nuevos. Backward-compatible: el frontend antiguo simplemente ignora los
--     campos extras.
...
COMMENT ON COLUMN partitions.work_start_v2 IS 'Inicio del slice v2 en basePairs (rp∈[20,50]); NULL hasta claim_v2_slice';
COMMENT ON COLUMN partitions.work_end_v2   IS 'Fin del slice v2; NULL hasta claim_v2_slice';
...
BEGIN
  ...
  IF v_v1_in_range THEN
    -- ─── CASO A: usuario sano, Lectura B (extender) ───────────────────────
    ...
    UPDATE partitions
     ...
     RETURNING * INTO v_row;
  ELSE
    -- ─── CASO B: usuario roto / nuevo, slice limpio de p_target_total ─────
    -- Reseteamos work_start/work_end a un slice fresco en zona v2 y dejamos
    -- work_start_v2/end_v2 NULL (no se necesitan: el work block es solo este).
    v_v2_start := v_zone_top;
    v_v2_end   := LEAST(v_zone_top + p_target_total, p_v2_max_idx);

    UPDATE partitions
       SET calib_v       = 2,
           work_start    = v_v2_start,
           work_end      = v_v2_end,
           work_start_v2 = NULL,
           work_end_v2   = NULL
     WHERE partitions.device_id = p_device_id
